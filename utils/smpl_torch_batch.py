...
from utils.smplx.body_models import SMPLX

class SMPLXModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.model = SMPLX(model_path, ext='pkl').to(device)
        self.faces = self.model.faces
        self.joint_regressor_halpe = torch.from_numpy(np.load('data/smpl/J_regressor_halpe_smplx.npy')).to(device)
    ...
    def forward(self, betas, thetas, trans, gR=None, halpe=False):
        ...
        output = self.model(betas, global_orient, body_pose, transl=transl)

        if halpe:
            joints = self.joint_regressor_halpe @ output.vertices
...
class SMPLModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.J_halpe_regressor = torch.from_numpy(np.load('data/smpl/J_regressor_halpe.npy')).to(self.data_type).to(self.device)
        self.J_lsp_regressor = torch.from_numpy(np.load('data/smpl/J_regressor_lsp.npy')).to(self.data_type).to(self.device)
